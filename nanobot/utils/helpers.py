...
def current_time_str(timezone: str | None = None, include_day=True) -> str:
    ...
    if len(offset) == 5 and offset.endswith("00"):
        offset = offset[:3]  # e.g. +02 instead of +0200
    offset_fmt = f"{offset[:3]}:{offset[3:]}" if len(offset) == 5 else offset
    tz_name = timezone or (time.strftime("%Z") or "UTC")
    if tz_name != "UTC":
        tz_name += f", UTC{offset_fmt}"
    date_format = "%Y-%m-%d %H:%M"
    if include_day:
        date_format += " (%A)"
    return f"{now.strftime(date_format)} ({tz_name})"
