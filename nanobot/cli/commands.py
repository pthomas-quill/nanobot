    ...
    if config_path.exists():
        if wizard:
            ...
            if typer.confirm("Overwrite?"):
                ...
                config = load_config(config_path,interpolate_env_vars=False) # Load without interpolation to preserve literal env var references
                config = _apply_workspace_override(config)
                save_config(config, config_path)
...
def _make_provider(config: Config):
    ...
    if backend == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.nanobot/config.yaml under providers.azure_openai section")
