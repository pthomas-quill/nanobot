...
class TestReadFileTool:
    ...
    @pytest.mark.asyncio
    async def test_basic_read_has_line_numbers_and_hash(self, tool, sample_file):
        result = await tool.execute(path=str(sample_file))
        hash1 = _hash_line("line 1")
        hash20 = _hash_line("line 20")
        assert f"1#{hash1}| line 1" in result
        assert f"20#{hash20}| line 20" in result

    @pytest.mark.asyncio
    async def test_offset_and_limit(self, tool, sample_file):
        result = await tool.execute(path=str(sample_file), offset=5, limit=3)
        hash5 = _hash_line("line 5")
        hash7 = _hash_line("line 7")
        hash8 = _hash_line("line 8")
        assert f"5#{hash5}| line 5" in result
        assert f"7#{hash7}| line 7" in result
        assert f"8#{hash8}| line 8" not in result
