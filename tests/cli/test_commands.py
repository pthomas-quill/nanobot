...
def mock_paths():
    ...
    with patch("nanobot.config.loader.get_config_path") as mock_cp, \
        ...
        mock_lc.side_effect = lambda _config_path=None, interpolate_env_vars=True: Config()

        def _save_config(config: Config, config_path: Path | None = None):
            target = config_path or config_file
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(yaml.safe_dump(config.model_dump(by_alias=True)), encoding="utf-8")
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    saved = Config.model_validate(yaml.safe_load(config_path.read_text(encoding="utf-8")))
...
def test_agent_hints_about_deprecated_memory_window(mock_agent_runtime, tmp_path):
    ...
    config_file.write_text(yaml.safe_dump({"agents": {"defaults": {"memoryWindow": 42}}}))
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanobot.config.loader.load_config", lambda _path=None,_interp=True: config)
    monkeypatch.setattr(
